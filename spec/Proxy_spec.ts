import {ProxyFactory, PropertyGetter, ProxyBuilder} from '../src/Proxy';
import {Person,SimplePerson,Service} from './SampleTypes';
import { SingletonLifetimeManager } from '../src';
describe('ProxyFactory',()=>{
    it('It creates a proxyFactory if not exist for the given key',()=>{
    ...
    it('uses a wildcard to match any type',()=>{
        ...
        var bld =fac.builder('*');    //wildcard proxy
        bld.get('serviceName').instead(()=>'fake service');

        var serviceBuilder = fac.getBuilder(Service);
        let svc = new Service();
        svc = serviceBuilder.proxy(svc);
        expect(svc.serviceName).toBe('fake service');
    });

    it('prefers explicit proxies to wildcards',()=>{

        var fac = new ProxyFactory();
        fac.builder('*').get('*').instead(()=>null);
        fac.builder(SimplePerson).get('lastname').instead(()=>null);  //pass through proxy
...
describe('Proxy Builder',()=>{
    ...
    describe('fns',()=>{
        it('returns fn unmodified if not configured',()=>{
            var b = new ProxyBuilder();
            var spp =b.proxy(new SimplePerson({firstname:'Thomas',lastname:'Siegfried'}));
            var s =spp.Announce();
