...
export class ProxyBuilder{
    ...
    private createHandler():any{
        return {
            get:(obj:any,prop:string)=>{
                ...
                else{   //value is not a function, look for getter wrappers
                    var getter = this._getters.get(prop);
                    if(!getter)
                        getter=this._getters.get(WILDCARD); //check for wildcard getter proxy
                    if(getter)
            ...
            set:(obj:any,prop:string,val:any)=>{
                ...
                if(!setter)
                    setter = this._setters.get(WILDCARD); //check for wildcard getter proxy
                if(setter)
    ...
    proxy<T extends Object>(obj:T){
        ...
        return new Proxy(obj,this.createHandler()) as T;
    }
